...
#define MAP_VALUE(idx, base, pos) \
  map[(idx)] = map[base] + (pos)

static void check_permutation(wchar_t *map) {
...
void vn_map_export(size_t n, const wchar_t *a, const char *name) {
  ...
  printf("\nconst size_t %s_len = sizeof(%s)/sizeof(%s[0]);\n", name, name, name);
  printf("#define VNCHMAP(c) \\\n"
         "   ((c) >= %s_len? (c): %s[(c)])\n", name, name);
}